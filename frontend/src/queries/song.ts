...
export const useSongsByArtist = (artistId: number, page: number = 1, limit: number = 10, search: string = '') => {
  return useQuery({
    queryKey: ['songs', artistId, page, limit, search],
    queryFn: () => songService.getSongsByArtist(artistId, page, limit, search),
    enabled: !!artistId,
...
export const useUpdateSong = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.data.artist_id) {
        queryClient.invalidateQueries({ queryKey: ['songs', variables.data.artist_id] })
      } else {
